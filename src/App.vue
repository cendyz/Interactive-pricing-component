<template>
	<div class="absolute top-0 left-0 w-full h-[26em] bg-paleBlue rounded-bl-[7em] opacity-20"></div>
	<main class="flex flex-col py-[7em] px-[1em] container relative">
		<div class="relative flex flex-col gap-y-[.6em] text-center">
			<h1 class="font-manrope800 relative z-10 text-[1.2rem]">Simple, traffic-based pricing</h1>
			<p class="relative z-10 text-[.8rem] px-[6em] text-grayishBlue font-manrope600">
				Sign-up for our 30-day trial. No credit card required.
			</p>
			<img :src="circles" alt="" class="absolute left-1/2 top-1/2 -translate-x-[50%] -translate-y-[50%]" />
		</div>
		<div
			...
			<div class="flex justify-center md:justify-between items-center w-full">
				<h2 class="uppercase text-grayishBlue font-manrope600 text-[.8rem]">100k pageviews</h2>
			...
			<div class="flex items-center mt-[1.7em]">
				<p class="capitalize text-[.8rem] text-grayishBlue font-manrope600">monthly billing</p>
				<button
					type="button"
		...
		<div
			...
			<div class="">
				<div class="flex items-center gap-x-[1.1em]" v-for="item in prosData" :key="nanoid()">
					<img :src="check" alt="" />
...
const updateTrackBackground = (): void => {
	const actualValue = data.default / data.max * 100
	document.documentElement.style.setProperty('--slider-fill', `${actualValue}%`)
}
